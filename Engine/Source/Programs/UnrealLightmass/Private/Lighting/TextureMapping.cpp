...
void FStaticLightingSystem::ProcessTextureMapping(FStaticLightingTextureMapping* TextureMapping)
{
	...
	TextureMapping->LightMapData = new FGatheredLightMapData2D(TextureMapping->CachedSizeX, TextureMapping->CachedSizeY);

	TextureMapping->LightMapData->bHasSkyShadowing = HasSkyShadowing();
	...
	TextureMapping->TexelToVertexMap = new FTexelToVertexMap(TextureMapping->CachedSizeX, TextureMapping->CachedSizeY);
	...
	SetupTextureMapping(TextureMapping, *TextureMapping->LightMapData, TexelToVertexMap, TexelToCornersMap, MappingContext, bDebugThisMapping);
	...
	if (bCalculateThisMapping)
	{
		...
		if (!bCalculateDirectLightingFromPhotons)
		{
			...
			for (int32 LightIndex = 0; LightIndex < TextureMapping->Mesh->RelevantLights.Num(); LightIndex++)
			{
				...
				if ( ShadowSettings.bUseZeroAreaLightmapSpaceFilteredLights)
				{
					...
					CalculateDirectLightingTextureMappingFiltered(TextureMapping, MappingContext, *TextureMapping->LightMapData, TextureMapping->ShadowMaps, TexelToVertexMap, bDebugThisMapping, Light);
				}
				else 
				{
						...
						&& ShadowSettings.bAllowSignedDistanceFieldShadows)
					{
						if (Light->LightFlags & GI_LIGHT_USE_AREA_SHADOWS_FOR_SEPARATE_SHADOW_FACTOR)
						{
							...
							CalculateDirectAreaLightingTextureMapping(TextureMapping, MappingContext, *TextureMapping->LightMapData, ShadowMapData, TexelToVertexMap, bDebugThisMapping, Light);
							...
							if (ShadowMapData)
							{
								...
								TextureMapping->SignedDistanceFieldShadowMaps.Add(Light, ConvertedShadowMapData);
							}
						}
						else
						{
							...
							if (bUseTextureSpaceDistanceFieldGeneration)
							{
								...
								CalculateDirectSignedDistanceFieldLightingTextureMappingTextureSpace(TextureMapping, MappingContext, *TextureMapping->LightMapData, TextureMapping->SignedDistanceFieldShadowMaps, TexelToVertexMap, TexelToCornersMap, bDebugThisMapping, Light);
							}
							else
							{
								// Experimental method that avoids artifacts due to lightmap seams
								CalculateDirectSignedDistanceFieldLightingTextureMappingLightSpace(TextureMapping, MappingContext, *TextureMapping->LightMapData, TextureMapping->SignedDistanceFieldShadowMaps, TexelToVertexMap, TexelToCornersMap, bDebugThisMapping, Light);
							}
						}
					}
					else if (Light->UseStaticLighting())
					{
						...
						{
							TextureMapping->LightMapData->AddLight(Light);
						}
					}
				}
			}
		...
		if (bCalculateDirectLightingFromPhotons)
		{
			CalculateDirectLightingTextureMappingPhotonMap(TextureMapping, MappingContext, *TextureMapping->LightMapData, TextureMapping->ShadowMaps, TexelToVertexMap, bDebugThisMapping);
		}
		MappingContext.Stats.DirectLightingTime += FPlatformTime::Seconds() - DirectLightingStartTime;

		CalculateIndirectLightingTextureMappingGPU(TextureMapping, false);
	}
#else
...
void FStaticLightingSystem::PostprocessTextureMapping(FStaticLightingTextureMapping* TextureMapping)
{
	bool bDebugThisMapping = false;
#if ALLOW_LIGHTMAP_SAMPLE_DEBUGGING
	bDebugThisMapping = TextureMapping == Scene.DebugMapping;
	...
	if (bCalculateThisMapping)
	{
		...
		ViewMaterialAttributesTextureMapping(TextureMapping, MappingContext, *TextureMapping->LightMapData, TexelToVertexMap, bDebugThisMapping);
		ColorInvalidLightmapUVs(TextureMapping, *TextureMapping->LightMapData, bDebugThisMapping);
	...
	PadTextureMapping(TextureMapping, *TextureMapping->LightMapData, PaddedLightMapData, TextureMapping->ShadowMaps, TextureMapping->SignedDistanceFieldShadowMaps);
	TextureMapping->LightMapData->Empty();
	...
	StaticLightingLink->Element.ShadowMaps = TextureMapping->ShadowMaps;
	StaticLightingLink->Element.SignedDistanceFieldShadowMaps = TextureMapping->SignedDistanceFieldShadowMaps;
	...
	delete TextureMapping->LightMapData;
	delete TextureMapping->TexelToVertexMap;
