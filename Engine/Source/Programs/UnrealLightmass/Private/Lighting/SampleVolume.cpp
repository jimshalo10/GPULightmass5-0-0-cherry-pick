...
void FStaticLightingSystem::ProcessVolumeSamplesTask(const FVolumeSamplesTaskDescription& Task)
{
	...
	PrecomputedUpperSamples.AddZeroed(Task.NumSamples);
	PrecomputedLowerSamples.AddZeroed(Task.NumSamples);
	WorldPositions.AddZeroed(Task.NumSamples);
	...
	for (int32 SampleIndex = Task.StartIndex; SampleIndex < Task.StartIndex + Task.NumSamples; SampleIndex++)
	{
		...
		WorldPositions[SampleIndex - Task.StartIndex] = CurrentSample.GetPosition();
	}

	CalculateVolumeSampleListGPU(WorldPositions, PrecomputedUpperSamples, PrecomputedLowerSamples);
	...
	for (int32 SampleIndex = Task.StartIndex; SampleIndex < Task.StartIndex + Task.NumSamples; SampleIndex++)
	{
			...
			&& (!PhotonMappingSettings.bUsePhotonMapping || PhotonMappingSettings.bUseFinalGathering))
		{
			...
			CalculateVolumeSampleIncidentRadiance(UniformHemisphereSamples, UniformHemisphereSampleUniforms, MaxUnoccludedLength, VertexOffsets, CurrentSample, BackfacingHitsFraction, Unused, RandomStream, MappingContext, PrecomputedUpperSamples[SampleIndex - Task.StartIndex], PrecomputedLowerSamples[SampleIndex - Task.StartIndex], true, bDebugSamples);
