...
const FStaticLightingMapping* FLightmassExporter::FindMappingByGuid(FGuid FindGuid) const
{
	...
	for( auto CurrentMapping : StaticMeshTextureMappings )
	{
		if (CurrentMapping->GetLightingGuid() == FindGuid)
		{
			return CurrentMapping;
	...
	for( auto CurrentMapping : VolumeMappings )
	{
		if (CurrentMapping->GetLightingGuid() == FindGuid)
		{
			return CurrentMapping;
...
void FLightmassExporter::WriteMeshInstances( int32 Channel )
{
	...
	int32 NextId = 0;
	for( auto SMLightingMesh : StaticMeshLightingMeshes )
	{
...
void FLightmassExporter::WriteMappings( int32 Channel )
{
	...
	for (auto SMTextureMapping : StaticMeshTextureMappings)
	{
	...
	for ( auto VolumeMapping : VolumeMappings )
	{
...
#error "Unknown Lightmass platform"
	...
	if (!bOnlyBuildVisibility)
	{
		...
		for (auto SMTextureMapping : Exporter->StaticMeshTextureMappings)
		{
